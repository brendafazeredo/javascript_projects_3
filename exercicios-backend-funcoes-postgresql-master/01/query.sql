...
select categoria, sum(estoque) as "estoque total" from farmacia where categoria = 'black' or categoria = 'blue' group by categoria;
...
select categoria, sum(estoque) as "estoque total" from farmacia where categoria IS NOT NULL group by categoria;

-- 7 - Faça uma query que retorna a soma do estoque disponível dos medicamentos sem categoria na tabela farmacia.

select categoria, sum(estoque) as "estoque total" from farmacia where categoria IS NULL group by categoria;
